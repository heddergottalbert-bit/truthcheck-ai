...
let huidigUitleg = "";
let huidigBronnen = [];
let huidigStrafbaar = { gevonden: false, categorieen: [] };
let popupOpen = false;
...
// ── Strafbaar icoontje ───────────────────────────────────────
const strafbaarKnop = document.createElement("div");
strafbaarKnop.id = "tc-strafbaar-knop";
strafbaarKnop.style.cssText = `
  position: fixed;
  width: 28px;
  height: 28px;
  border-radius: 50%;
...
`;
strafbaarKnop.innerHTML = "⚠️";
strafbaarKnop.title = "Strafbare content gedetecteerd";
document.body.appendChild(strafbaarKnop);

// Strafbaar mini-popup
const strafbaarPopup = document.createElement("div");
strafbaarPopup.id = "tc-strafbaar-popup";
strafbaarPopup.style.cssText = `
  position: fixed;
  width: 220px;
  ...
  font-family: Georgia, serif;
`;
document.body.appendChild(strafbaarPopup);

function positioneerStrafbaarKnop() {
  const rechts = parseInt(knop.style.right) || 20;
  const onder = parseInt(knop.style.bottom) || 20;
  strafbaarKnop.style.right = (rechts + 18) + "px";
  strafbaarKnop.style.bottom = (onder - 34) + "px";
}

function updateStrafbaarIcoontje(strafbaar) {
  if (!strafbaar || !strafbaar.gevonden || strafbaar.categorieen.length === 0) {
    strafbaarKnop.style.display = "none";
    strafbaarPopup.style.display = "none";
    return;
  }

  positioneerStrafbaarKnop();
  strafbaarKnop.style.display = "flex";

  // Bouw de mini-popup inhoud
  const categorieenHTML = strafbaar.categorieen.map(c => `
    <div style="
      display:flex; align-items:center; justify-content:space-between;
      background:rgba(231,76,60,0.12); border-left:3px solid ${c.kleur};
      border-radius:6px; padding:5px 8px; margin-bottom:5px;
    ">
      <span style="font-size:11px; color:${c.kleur}; font-family:Georgia,serif;">
        ${c.label}
      </span>
      <span style="font-size:10px; color:#aaa; font-family:Georgia,serif;">
        ${c.score}%
      </span>
    </div>
  `).join("");

  strafbaarPopup.innerHTML = `
    <div style="font-size:9px; letter-spacing:2px; color:#e74c3c;
      text-transform:uppercase; margin-bottom:10px; font-family:Georgia,serif;">
      ⚠️ Strafbare content
    </div>
    ${categorieenHTML}
    <button id="tc-strafbaar-sluit" style="
  ...
  document.getElementById("tc-strafbaar-sluit").onclick = () => {
    strafbaarPopup.style.display = "none";
  };
}

let strafbaarPopupOpen = false;
strafbaarKnop.addEventListener("click", (e) => {
  e.stopPropagation();
  strafbaarPopupOpen = !strafbaarPopupOpen;
  if (strafbaarPopupOpen) {
    const knopRect = strafbaarKnop.getBoundingClientRect();
    strafbaarPopup.style.right = knop.style.right;
    strafbaarPopup.style.bottom = (parseInt(knop.style.bottom) + 74) + "px";
    strafbaarPopup.style.display = "block";
  } else {
    strafbaarPopup.style.display = "none";
  }
});

strafbaarKnop.addEventListener("mouseenter", () => {
  strafbaarKnop.style.transform = "scale(1.15)";
});
strafbaarKnop.addEventListener("mouseleave", () => {
  strafbaarKnop.style.transform = "scale(1)";
});

document.addEventListener("click", (e) => {
  if (!strafbaarPopup.contains(e.target) && e.target !== strafbaarKnop) {
    strafbaarPopup.style.display = "none";
    strafbaarPopupOpen = false;
  }
});
...
function updatePopup(score, oordeel, uitleg, bronnen, strafbaar) {
  const kleur = getKleur(score);
...
menu.querySelectorAll("[data-kleur]").forEach(btn => {
  btn.onclick = () => {
    ...
    updateMiniBarometer(huidigScore);
    if (popupOpen) updatePopup(huidigScore, huidigOordeel, huidigUitleg, huidigBronnen, huidigStrafbaar);
    slaInstellingenOp();
  };
});
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        if (response && response.status === "success") {
          ...
          huidigStrafbaar = response.strafbaar || { gevonden: false, categorieen: [] };

          updateMiniBarometer(huidigScore);
          updateStrafbaarIcoontje(huidigStrafbaar);
          ...
          if (popupOpen) {
            updatePopup(huidigScore, huidigOordeel, huidigUitleg, huidigBronnen, huidigStrafbaar);
          }
        }
      }
    );
...
// ── Vertraagde reactiecheck (voor dynamisch geladen reacties) ─
function leesReacties() {
  const reactieSelectors = [
    ".nujij__comment-body",
    ".nujij__comment-wrapper",
    "[class*='comment-body']",
    "[class*='comment__body']",
    "[class*='comment__text']",
  ...
  for (const selector of reactieSelectors) {
...
function startReactiecheck() {
  const reactieTekst = leesReacties();
  console.log("[TruthCheck] startReactiecheck aangeroepen, reactieTekst lengte:", reactieTekst.length);
  if (!reactieTekst || reactieTekst.trim().length < 50) {
    console.log("[TruthCheck] Te weinig reactietekst gevonden, stop.");
    return;
  }

  if (chrome.runtime && chrome.runtime.sendMessage) {
    console.log("[TruthCheck] Stuur reacties naar background.js...");
    chrome.runtime.sendMessage(
      { action: "start_check_reacties", reactieTekst },
      (response) => {
        if (chrome.runtime.lastError) {
          console.log("[TruthCheck] Fout:", chrome.runtime.lastError);
          return;
        }
        console.log("[TruthCheck] Reactie van background:", JSON.stringify(response));
        if (response && response.status === "success") {
          huidigStrafbaar = response.strafbaar || { gevonden: false, categorieen: [] };
          updateStrafbaarIcoontje(huidigStrafbaar);
        }
...
function startCheck() {
  ...
  // Artikeltekst — voor feitencheck
  const artikelEl = document.querySelector("article")
    || document.querySelector("[class*='article']")
    || document.querySelector("[class*='content']")
    || document.querySelector("main");
  const artikelTekst = artikelEl
    ? artikelEl.innerText.substring(0, 2000).toLowerCase()
    : document.body.innerText.substring(0, 2000).toLowerCase();

  // Reacties — alleen voor strafbare content check
  let reactieTekst = leesReacties();

  // Paginatekst = artikel + reacties samen (voor strafbare check)
  const paginaTekst = (artikelTekst + "\n" + reactieTekst).substring(0, 4000);
  ...
  // Start vertraagde reactiecheck — reacties laden vaak na de pagina
  setTimeout(startReactiecheck, 6000);
  setTimeout(startReactiecheck, 12000);

  if (chrome.runtime && chrome.runtime.sendMessage) {
    chrome.runtime.sendMessage(
      { action: "start_check", text, domein, paginaTekst, artikelTekst, reactieTekst, zoekContext },
      (response) => {
        if (chrome.runtime.lastError) return;
