...
// ── GPT-gebaseerde strafbare content check ───────────────────
async function controleerStrafbareContent(tekst) {
  if (!tekst || tekst.trim().length < 20) {
    return { gevonden: false, categorieen: [] };
  }

  const CATEGORIE_NAAR_INFO = {
    "haatzaaien":      { label: "Haatzaaien / discriminatie", kleur: "#e74c3c" },
    "bedreiging":      { label: "Bedreiging / intimidatie",   kleur: "#e74c3c" },
    "geweldsverheerlijking": { label: "Verheerlijking van geweld", kleur: "#c0392b" },
    "illegale_handel": { label: "Illegale handel",            kleur: "#e67e22" },
    "oplichting":      { label: "Oplichting / fraude",        kleur: "#e67e22" },
    "csam":            { label: "⚠️ CSAM signaal",            kleur: "#c0392b" }
  };
    ...
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `Je bent een Nederlandse content moderator gespecialiseerd in het detecteren van strafbare en schadelijke uitingen.

Beoordeel de tekst op de aanwezigheid van:
- haatzaaien: discriminatie, racisme, xenofobie, aanzetten tot haat op basis van nationaliteit, etniciteit, religie of afkomst. Ook subtiele vormen zoals "stuur ze terug", "ons geld", verwijzingen naar vluchtelingen als bedreiging.
- bedreiging: directe of indirecte bedreigingen of intimidatie van personen of groepen
- geweldsverheerlijking: geweld toejuichen, aanmoedigen of verheerlijken
- illegale_handel: aanbod of instructies voor drugs, wapens, gestolen goederen
- oplichting: nep-aanbiedingen, fraude, misleiding met financieel doel
- csam: seksuele content gerelateerd aan minderjarigen

BELANGRIJK: Journalistieke tekst over misdaad en rechtbankverslagen zijn NOOIT strafbaar. Maar gebruikersreacties en commentaren WEL als ze discrimineren of haatzaaien.

Geef ALLEEN een JSON object terug, geen extra tekst:
{"gevonden": false, "categorieen": []}

Of als er iets gevonden is:
{"gevonden": true, "categorieen": [{"naam": "haatzaaien", "zekerheid": 75}]}

Zekerheid is 0-100. Voeg toe als zekerheid >= 40.`
          },
          {
            role: "user",
            content: "Beoordeel deze tekst:\n\n" + tekst.substring(0, 3000)
          }
        ],
        max_tokens: 200,
        temperature: 0
    ...
    const tekst_response = data.choices?.[0]?.message?.content || "";

    let resultaat;
    try {
      resultaat = JSON.parse(tekst_response.replace(/```json|```/g, "").trim());
    } catch {
      return { gevonden: false, categorieen: [] };
    }

    if (!resultaat.gevonden || !resultaat.categorieen?.length) {
      return { gevonden: false, categorieen: [] };
    }

    const gevondenCategorieen = resultaat.categorieen
      .filter(c => c.zekerheid >= 40)
      .map(c => {
        const info = CATEGORIE_NAAR_INFO[c.naam] || { label: c.naam, kleur: "#e74c3c" };
        return {
          categorie: c.naam,
          label: info.label,
          kleur: info.kleur,
    ...
    return {
      gevonden: gevondenCategorieen.length > 0,
      categorieen: gevondenCategorieen
    };

  } catch (err) {
    return { gevonden: false, categorieen: [], fout: err.message };
  }
}

// ── Hoeveel punten aftrekken op basis van ernst ──────────────
function berekenScoreAftrek(categorieen) {
  if (!categorieen || categorieen.length === 0) return 0;
  const zwaarste = categorieen[0];
  if (zwaarste.score >= 80) return 50;
  if (zwaarste.score >= 60) return 35;
  if (zwaarste.score >= 40) return 20;
  return 10;
}

chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {

  // ── Alleen reacties checken op strafbare content ─────────────
  if (request.action === "start_check_reacties") {
    const reactieTekst = request.reactieTekst || "";
    controleerStrafbareContent(reactieTekst).then(strafbaar => {
      sendResponse({ status: "success", strafbaar });
    });
    return true;
  }
  ...
  if (request.action === "start_check") {
    ...
    // Phishing gedetecteerd — stuur direct terug, geen moderations nodig
    if (phishing.actief) {
      sendResponse({
        status: "success",
        ...
        bronnen: [],
        phishing: phishing,
        strafbaar: { gevonden: false, categorieen: [] }
      });
      return true;
    }

    // Voor emails: alleen moderations check
    if (request.isEmail) {
      const mailTekst = request.paginaTekst || "";
      controleerStrafbareContent(mailTekst).then(strafbaar => {
        sendResponse({
          status: "success",
          score: strafbaar.gevonden ? Math.max(10, 75 - berekenScoreAftrek(strafbaar.categorieen)) : 75,
          oordeel: strafbaar.gevonden ? "Verdachte inhoud gedetecteerd" : "Geen gevaar gedetecteerd",
          uitleg: strafbaar.gevonden
            ? "Deze e-mail bevat mogelijk strafbare of schadelijke inhoud."
            : "Geen phishing of schadelijke signalen gevonden.",
          bronnen: [],
    ...
    // Websites — feitencheck + strafbare content parallel
    const zoekTerm = request.text +
      (request.zoekContext ? " " + request.zoekContext : "");

    // Feitencheck gebruikt alleen artikeltekst
    // Strafbare content check gebruikt reacties (prioriteit) + paginatekst
    const reactieTekst = request.reactieTekst || "";
    const moderatieTekst = reactieTekst.length > 100
      ? reactieTekst.substring(0, 3000)
      : ((request.paginaTekst || "") + " " + (request.text || "")).substring(0, 3000);

    Promise.all([
      // Tavily feitencheck — zoekt op basis van artikeltitel
      ...
      // OpenAI strafbare content check — kijkt naar reacties
      controleerStrafbareContent(moderatieTekst)
    ])
    .then(([tavilyData, strafbaar]) => {
      const paginaDomein = request.domein || "";
...
{"score": 50, "oordeel": "Onbekend", "uitleg": "Er zijn geen betrouwbare bronnen gevonden.", "bronRelevant": false}`
        ...
        .then(aiData => {
          ...
          // Feitencheck score staat volledig los van strafbare content
          sendResponse({
            status: "success",
            score: result.score,
