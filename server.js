...
app.post('/api/youtube', controleerApiKey, rateLimiter, async (req, res) => {
  try {
    ...
    if (contentType === 'entertainment') {
      ...
      // AI-score berekening — nieuwe schaal:
      // 0%  = geen signaal
      // 25% = alleen YouTube label aangevinkt
      // 50% = alleen tekstuele patronen in beschrijving
      // 75% = YouTube label + tekstuele patronen
      // 100% = YouTube label + patronen + OpenAI bevestigt (niet hier, want geen OpenAI call)
      const AI_PATRONEN = [
        // Directe AI-vermeldingen
        ...
        'gecreëerd door ai', 'gegenereerd door ai', 'generated by ai',
        'ai kunst', 'ai art', 'ai video', 'ai beelden', 'ai visuals',
        'ai animatie', 'ai animation', 'ai content', 'ai-beelden',
        // Bekende AI-tools
        'midjourney', 'stable diffusion', 'dall-e', 'dall·e', 'sora',
        'runway', 'runwayml', 'pika', 'kling', 'luma', 'leonardo ai',
        'digen', 'digen ai', 'ideogram', 'adobe firefly', 'canva ai',
        ...
        'synthetisch', 'synthetic media', 'deepfake', 'face swap',
      ...
      let lokaleAiScore = 0;
      if (isAiContent && heeftTekstuelePatronen) lokaleAiScore = 75;
      else if (isAiContent) lokaleAiScore = 25;
